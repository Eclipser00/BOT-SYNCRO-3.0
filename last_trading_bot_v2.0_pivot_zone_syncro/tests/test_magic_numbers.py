...
class DoubleBuyStrategy(PivotZoneTestStrategy):
    """Estrategia que emite DOS señales BUY para el mismo símbolo en el mismo generate_signals."""
    ...
    def generate_signals(self, data_by_timeframe: dict[str, pd.DataFrame]) -> list[Signal]:
        ...
        stop = entry - 0.001
        tp = entry + 0.001
        # Retorna intencionalmente dos señales BUY idénticas en el mismo ciclo
        return [
            Signal(
                symbol=symbol,
                strategy_name=self.name,
...
def test_no_duplicate_orders_same_cycle():
    """Verifica que si una estrategia emite dos señales BUY para el mismo ticker
    en el mismo generate_signals, el bot solo ejecuta una orden y abre una posición."""
    broker = FakeBroker()
    strategy = DoubleBuyStrategy(name="double_buy_strategy", broker_client=broker)
    bot, broker = _build_bot(strategy)

    # Un solo run_once — la estrategia emitirá 2 BUY en el mismo ciclo
    bot.run_once(now=datetime.now(timezone.utc))

    assert len(broker.orders_sent) == 1, (
        f"Se esperaba 1 orden enviada, pero se enviaron {len(broker.orders_sent)}. "
        "El bot no está filtrando señales BUY duplicadas dentro del mismo ciclo."
    )
    assert len(broker.open_positions) == 1, (
        f"Se esperaba 1 posición abierta, pero hay {len(broker.open_positions)}. "
        "El bot abrió múltiples posiciones por señales duplicadas en el mismo ciclo."
    )
